...
class UniTensor():
    ...
    def __init__(self, bonds, N_inbond=None ,labels=None, device=torch.device("cpu"),dtype=torch.float64,torch_tensor=None,check=True, is_diag=False, requires_grad=False, name="",braket=None,sym_mappers=None):
        ...
        if not self.is_symm:
            ...
            if sym_mappers is None:
                ...
                self._bra_mapper_blks = -np.ones((len(bq_in),2)),astype(np.int)
                ...
                self._bra_mapper_blks = copy.copy(sym_mappers[2])
                self._bra_invmapper_blks = copy.copy(sym_mappers[3])
                self._ket_mapper_blks = copy.copy(sym_mappers[4])
                self._ket_invmapper_blks = copy.copy(sym_mappers[5])
    ...
    def __eq__(self,rhs):
        ...
        if isinstance(rhs,self.__class__):
            ...
            if self.is_symm:
                ...
                raise Exception("[Develope]")
                iss = iss and (len(self.Storage) == len(rhs.Storage))
                if not iss:
                    return False
                for b in range(len(self.Storage)):
                    iss = iss and (self.Storage[b].shape == rhs.Storage[b].shape)
            else:
    ...
    def shape(self):
        ...
        if self.is_symm:
            ## what to return ?
            raise Exception("[DEvelope]")
            return [ self.Storage[b].shape for b in range(len(self.Storage)) ]
        else:
            if self.is_diag:
    ...
    def __pow__(self,other):
        if self.is_symm:
            ...
            tmp = UniTensor(bonds=self.bonds,\
                            ...
                            N_inbond=self.N_inbond,\
                            check=False,\
                            sym_mappers=(self._mapper,self._inv_mapper,self._bra_mapper_blks,self._bra_invmapper_blks,self._ket_mapper_blks,self._ket_invmapper_blks),\
    ...
    def GetTotalQnums(self):
        ...
        #virtual_cb-in
        cb_inbonds = copy.deepcopy(self.bonds[np.argwhere(self.braket==BondType[BD_BRA])])
        in_all = cb_inbonds[0]
        if len(cb_inbonds)>1:
            in_all.combine(cb_inbonds[1:])

        cb_outbonds = copy.deepcopy(self.bonds[np.argwhere(self.braket==BondType[BD_KET])])
