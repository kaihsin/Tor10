...
class UniTensor():
    ...
    def __init__(self, bonds, labels=None, device=torch.device("cpu"),dtype=torch.float64,torch_tensor=None,check=True, is_diag=False, ,is_blockform=False, requires_grad=False, name="", BlockMapper=None, BlockQnum=None):
        """
        ...
        Public Args:
            ...
            is_blockform:
                This states if the current UniTensor is stored as block sparsed form. If True, the Storage will be the list of blocks.
                Note that if is_blockform=True, then the UniTensor should be a symmetry tensor. 
            ...
            torch_tensor : 
                This is the internal arguments in current version. It should not be directly use, otherwise may cause inconsistence with Bonds and memory layout. 
                    *For Developer:
                        ...
                        > for each function, the is_blockform will be check first, then the is_diag will be check. 
        ...
        ## these are only set if is_blockform=True:
        if BlockMapper is None or BlockQnum is None:
            self._BlockMapper_in = None
            self._BlockMapper_out = None
            self._BlockQnum = None
        else:
            self._BlockMapper_in = copy.deepcopy(BlockMapper[0])
            self._BlockMapper_out = copy.deepcopy(BlockMapper[1])
            self._BlockQnum = copy.deepcopy(BlockQnum)
        ...
        if check:
            ...
            if self.bonds[0].qnums is not None:
                ...
                for s in range(self.bonds[0].nsym):
                    if len(np.unique([ bd.sym_types[s].__class__ for bd in self.bonds])) != 1:
                        raise TypeError("UniTensor.__init__","the symmetry types for every symmetry bonds must be consistent.")
                               
 
            if is_blockform:
                if self.bonds[0].qnums is None:
                    raise TypeError("UniTensor.__init__","is_blockform cannot set to True for non-symmetry tensor.")
                
                if BlockMapper is not None:
                    raise Exception("UniTensor.__init__","[WARNING] BlockMapper Should Not be set directly by the user! If you are sure it is save, set [check=False] to override")
                if BlockQnum is not None:
                    raise Exception("UniTensor.__init__","[WARNING] BlockMapper Should Not be set directly by the user! If you are sure it is save, set [check=False] to override")
        ...
        if torch_tensor is None:
            if self.is_blockform:
                ...
                tqin,tqout = self.GetTotalQnum()
                tqin_uni = np.unique(tqin,axis=0) 
                tqout_uni= np.unique(tqout,axis=0)
                
                Nsym = self.bonds[0].nsym
                dtype={'names':['f{}'.format(i) for i in range(Nsym)],
                       'formats':Nsym * [tqin_uni.dtype]}

                
                C = np.intersect1d(tqin_uni.view(dtype), tqout_uni.view(dtype))
                C = C.view(tqin_uni.dtype).reshape(-1, Nsym)                

                #Create block:
                tqin = tqin.view(dtype)
                tqout = tqout.view(dtype)
                self.Storage = []
                self._BlockMapper_in = []
                self._BlockMapper_out = []
                for blk in range(len(C)):
                    comm = np.array([C[blk]]).reshape(1,Nsym).view(dtype)
                    _,idx_in,_=np.intersect1d(tqin,comm,return_indices=True)
                    _,idx_out,_=np.intersect1d(tqout,comm,return_indices=True)
        ...
        if requires_grad:
            if self.is_blockform:
                for i in range(len(self.Storage)):
                    self.Storage[i].requires_grad_(True)
            else:
    ...
    def Todense(self):
        ...
        Set the UniTensor to dense matrix. 
            1. if the UniTensor is in block sparse form, this will transform it into a dense UniTensor.
            2. if the UniTensor is non-symmetry, but with is_diag = True where only the diagonal matrix is stored. this will trasform it into a dense UniTensor
        ...
        """
        if self.is_blockform=True:
            print("developing.")
            exit(1)
        elif self.is_diag==True:
    ...
    def to(self,device):
        ...
        if self.is_blockform:
            for blk in range(len(self.Storage)):
                self.Storage[blk] = self.Storage[blk].to(device)
        else:
    ...
    def Print_diagram(self):
        ...
        print("is_blockform: %s"%("True" if self.is_blockform else "False"))
    ...
    def __len__(self):
        """
            Get the len of the UniTensor.
            
            1. if UniTensor is symmetric, it will return the number of valid blocks.
            2. if UniTensor is non-symmetry, it will return the first dimension of the UniTensor.
            3. if UniTensor is non-symmetry with is_diag=True, it will return the # of diagonal element.
    ...
    def __eq__(self,rhs):
        ...
        if isinstance(rhs,self.__class__):
            if self.is_blockform != rhs.blockform:
                return False

            if self.is_blockform:
                iss = (len(self.Storage) == len(rhs.Storage))
                for s in range(len(self.Storage)):
                    iss = iss and (self.Storage[s].shape == self.Storage[s].shape)
            else:
                iss = (self.Storage.shape == rhs.Storage.shape) 
            iss = iss and (len(self.bonds) == len(rhs.bonds))
    ...
    def shape(self):
        ...
        """
        if self.is_blockform:
            return torch.Size([self.bonds.dim[x] for x in range(len(self.bonds))]
)
        elif self.is_diag:
    ...
    def __getitem__(self,key):
        if self.is_blockform:
            raise Exception("UniTensor[idx]","[ERROR] for symmetry UniTensor in sparse blockform, use GetBlock to get element.")
        return self.Storage[key]

    def __setitem__(self,key,value):
        if self.is_blockform:
            raise Exception("UniTensor[idx]","[ERROR] for symmetry UniTensor in sparse blockform, use PutBlock to set element.")
    ...
    def __add__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform==False:
                    raise Exception("UniTensor.__add__","[ERROR] cannot add sparse block form Tensor with densed tensor.")

                ##checking:
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__add__","[ERROR] try to add two sparse block form Tensors with different block form structure.")
                 
                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__add__","[ERROR] try to add two sparse block form Tensors with different qnum structure.")

                tmp = UniTensor(bonds = self.bonds,\
                                labels=self.labels,\
                                torch_tensor=[self.Storage[x] + other.Storage[x] for x in range(len(self.Storage))],\
                                check=False,\
                                is_blockform=True,BlockMapper=(self._BlockMapper_in,self._BlockMapper_out),BlockQnum=self._BlockQnum)
            ...
            if self.is_blockform:
                return UniTensor(bonds = self.bonds,\
                                 labels= self.labels,\
                                 torch_tensor=[self.Storage[x] + other for x in range(len(self.Storage))],\
                                 check=False,
                                 is_blockform=True,BlockMapper=(self._BlockMapper_in,self._BlockMapper_out),BlockQnum=self._BlockQnum)
            else:
                return UniTensor(bonds = self.bonds,\
    ...
    def __sub__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform==False:
                    raise Exception("UniTensor.__sub__","[ERROR] cannot add sparse block form Tensor with densed tensor.")
                ## checking :
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__sub__","[ERROR] try to add two sparse block form Tensors with different block form structure.")

                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__sub__","[ERROR] try to add two sparse block form Tensors with different qnum structure.")
    ...
    def __mul__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform=False:
                    raise Exception("UniTensor.__mul__","[ERROR] cannot add sparse block form Tensor with densed tensor.")

                ##checking:
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__mul__","[ERROR] try to add two sparse block form Tensors with different block form structure.")

                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__mul__","[ERROR] try to add two sparse block form Tensors with different qnum structure.")
    ...
    def __pow__(self,other):
        ...
        if self.is_blockform:
            return UniTensor(bonds=self.bonds,\
                             ...
                             check=False,\
                             is_diag=self.is_diag,\
                             BlockMapper=(self._BlockMapper_in,self._BlockMapper_out),BlockQnum=self._BlockQnum)
        else:
            return UniTensor(bonds=self.bonds,\
                             labels=self.labels,\
                             torch_tensor=self.Storage**other,\
    ...
    def __truediv__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform=False:
                    raise Exception("UniTensor.__truediv__","[ERROR] cannot truediv sparse block form Tensor with densed tensor.")

                ##checking:
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__truediv__","[ERROR] try to add two sparse block form Tensors with different block form structure.")
                 
                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__truediv__","[ERROR] try to add two sparse block form Tensors with different qnum structure.")
                ...
                if other.is_blockform is True:
                    raise TypeError("UniTensor.__truediv__","[ERROR] try to truediv sparse block form Tensor with dense tensor.")
            ...
            if self.is_blockform:
                return UniTensor(bonds=self.bonds,\
                                 ...
                                 check=False,\
                                 is_diag=self.is_diag,\
                                 BlockMapper=(self._BlockMapper_in,self.BlockMapper_out),BlockQnum=self._BlockQnum)
            else:
                return UniTensor(bonds=self.bonds,\
                                 labels=self.labels,\
                                 torch_tensor=self.Storage / other,\
    ...
    def __iadd__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform is False:
                    raise Exception("UniTensor.__iadd__","[ERROR] cannot iadd sparse blockform tensor with dense tensor")
               
                ##checking
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__iadd__","[ERROR] try to add two sparse block form Tensors with different block form structure.")

                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__iadd__","[ERROR] try to add two sparse block form Tensors with different qnum structure.")
    ...
    def __isub__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform is False:
                    raise Exception("UniTensor.__isub__","[ERROR] cannot isub sparse blockform tensor with dense tensor")
    ...
    def __imul__(self,other):
        if isinstance(other, self.__class__):
            if self.is_blockform:
                if other.is_blockform is False:
                    raise Exception("UniTensor.__imul__","[ERROR] cannot imul sparse blockform tensor with dense tensor")
                ##checking :
                if len(self.bonds) != len(other.bonds):
                    raise TypeError("UniTensor.__imul__","[ERROR] try to imul two sparse block form Tensors with different block form structure.")

                if not all(self.bonds[i]==other.bonds[i] for i in range(len(self.bonds))):
                    raise TypeError("UniTensor.__imul__","[ERROR] try to imul two sparse block form Tensors with different qnum structure.")
    ...
    def Permute(self,maper,N_inbond,by_label=False):
        ...
        if self.is_blockform:
            raise Exception("UniTensor.Permute","[Developing!!]")
    ...
    def Reshape(self,dimer,N_inbond,new_labels=None):
        ...
        if self.is_diag:
            raise Exception("UniTensor.Reshape","[ERROR] UniTensor.is_diag=True cannot be Reshape.\n"+
                                                "[Suggest] Call UniTensor.Todense()")
        if self.is_blockform:
            raise Exception("UniTensor.Reshape","[ERROR] UniTensor.is_blockform=True cannot be Reshape.\n")
    ...
    def PutBlock(self,block,*qnum):
        ...
        if self.bonds[0].qnums is None: 
            ...
            if self.is_blockform:
                ## check *qnum is exist:
                dtype={'names':['f{}'.format(i) for i in range(Nsym)],
                       'formats':Nsym * [self.BlockQnums.dtype]}
                
                _,qid,_= np.intersect1d(self.BlockQnums.view(dtype),np.array(qnum).reshape(1,Nsym).view(dtype),return_indices=True)
                if len(qid) != 1:
                    raise ValueError("UniTensor.PutBlock","[ERROR] No block in Tensor has the assigned qnum.")

                else:
                    ##checking:
                    qid = qid[0]
                    if isinstance(block,np.ndarray):
                        if self.Storage[qid].shape != tor.from_numpy(block).shape:
                            raise TypeError("UniTensor.PutBlock","[ERROR] The input block shape does not match.")
                        
                        self.Storage[qid] = copy.deepcopy(tor.from_numpy(block))

                    elif isinstance(block,self.Storage[0].__class__):
                        if self.Storage[qid].shape != block.shape:
                            raise TypeError("UniTensor.PutBlock","[ERROR] The input block shape does not match.")

                        self.Storage[qid] = copy.deepcopy(block)
                        ...
                        if block.is_blockform:  
                            raise TypeError("UniTensor.PutBlock","[ERROR] cannot input sparse blockform tensor as block.")
                        if block.is_diag:
                            self.Storage[qid] = copy.deepcopy(block.Storage).Todense()
                        else:
                            self.Storage[qid] = copy.deepcopy(block.Storage)
                        
                    else:
                        raise TypeError("UniTensor.PutBlock","[ERROR] the block can only be np.ndarray, UniTensor or %s"%(self.Storage[0].__class__))
                ...
                if isinstance(block,np.ndarray):
                    ...
                    if block.is_blockform:
                        raise Exception("UniTensor.PutBlock","[ERROR] cannot put a sparse blockform tensor as a block ")
                    self.Storage[np.ix_(i_in,i_out)] = block.Storage
    ...
    def GetBlock(self,*qnum):
        ...
        if self.bonds[0].qnums is None:
            ...
            if self.is_blockform:            
                ...
                _,qid,_= np.intersect1d(self.BlockQnums.view(dtype),np.array(qnum).reshape(1,Nsym).view(dtype),return_indices=True)
                if len(qid) != 1:
                    raise ValueError("UniTensor.GetBlock","[ERROR] No block in Tensor has the assigned qnum.")

                else:
                    return UniTensor(bonds =[Bond(BD_IN,dim=self.Storage[qid].shape[0]),Bond(BD_OUT,dim=self.Storage[qid].shape[1])],\
                                     labels=[1,2],\
                                     torch_tensor=self.Storage[qid],\
                                     check=False)
    ...
    def grad(self):
        ...
        if self.is_blockform:
            rets=[]
            for s in range(len(self.Storage)):
                if self.Storage[s].grad is None:
                    return None
                rets.append(self.Storage[s].grad)
            return UniTensor(bonds = copy.deepcopy(self.bonds),\
                             torch_tensor=rets,\
                             check=False,\
                             is_blockform=True,\
                             BlockMapper=(self._BlockMapper_in,self._BlockMapper_out),\
                             BlockQnum=self._BlockQnum)
